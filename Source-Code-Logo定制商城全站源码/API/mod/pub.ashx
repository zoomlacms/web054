                ...
                case "add":
                    {
                        ...
                        int contentId = DBCenter.Insert(pubMod.PubTableName, BLLCommon.GetFields(dr), BLLCommon.GetParas(dr), BLLCommon.GetParameters(dr));
                        retMod.result = contentId.ToString();
                        retMod.retcode = M_APIResult.Success;
                        //如果提交了设计需求,则发送邮件
                        //dt.Columns["PubTitle"].ColumnName = "name";
                        //dt.Columns["fhlx"].ColumnName = "SignType";
                        //dt.Columns["bpwz"].ColumnName = "SignText";
                        //dt.Columns["orther"].ColumnName = "CustomText";
                        if (PubID == 9)
                        {
                            StringWriter sw = new StringWriter();
                            HttpContext.Current.Server.Execute("/Common/MailTlp/Pub.aspx?ID=" + contentId, sw);
                            string title = "Thank you for the request";
                            string html = sw.ToString();
                            ZoomLa.Sns.EventDeal.SendToEmail(DataConvert.CStr(dr["Email"]), title, html, "pub email[" + contentId + "]");
                            ZoomLa.Sns.EventDeal.SendToEmail(ZoomLa.Components.SiteConfig.SiteInfo.WebmasterEmail, title, html, "pub email[" + contentId + "]");
                        }
                    }
                    break;
                case "edit":
