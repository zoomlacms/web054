...
fabHelper.init = function (json,cfg) {
    ...
    canvas.loadFromJSON(json, function () {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            //list[i].orgin = JSON.stringify(list[i]);
            //list[i].orgin = { scaleX: list[i].scaleX, scaleY: list[i].scaleY,left:list[i].left,top:list[i].top };
            list[i].orgin = { width: list[i].width, heigth: list[i].height };
            //fabHelper.cache[list[i].cacheKey] = JSON.stringify(list[i]);
            ...
            case "text":
                {
                    ...
                    $("#text_text").attr("maxlength", model.length);
                    $("#text_size").val(model.size);
                    $("#text_color").val(model.color);
                    $("#text_direction").val(model.direction);
                    $("#font_ul li").removeClass("active");
                    if (model.family) {
                        $("#font_ul li").each(function () {
                            var font = $(this).data("font");
                            if (model.family == font) { $(this).addClass("active"); }
                        });
                    }

                    //$("#text_scaleX").val(fabObj.scaleX);
                    //$("#text_scaleY").val(fabObj.scaleY);
                }
                break;
            case "sprite":
                {
        ...
        case "text":
            {
                ...
                model.direction = $("#text_direction").val();
                ...
                fabObj.setSrc(fabHelper.getTextUrl(model), function (textObj) {
                    ...
                    //如果宽度超出了限定,则自动调整(改为以图片宽度为准)
                    //var textWidth = parseInt((textObj.width * textObj.scaleX));
                    //var borderWidth = border.right - border.left;
                    //if (textWidth > borderWidth) {
                    //    textObj.set({ width: parseInt(borderWidth / textObj.scaleX) });
                    //}
                    ////如果距离+宽度超过了限制,则左对齐
                    //if ((textObj.left + textWidth) > border.right) {
                    //    textObj.set({ left: border.left });
                    //}
                    //textObj.setCoords();
...
fabHelper.getTextUrl = function (model) {
    ...
    if (model.width) { url += "&width=" + model.width; }
    return url;
}
//设置背景色或背景图片
...
fabHelper.text_orgin = function (type) {
    var fabObj = canvas.getActiveObject();
    var orgin = fabObj.orgin;
    fabObj.scaleX = orgin.scaleX;
    fabObj.scaleY = orgin.scaleY;

    fabObj.set({ left: orgin.left, top: orgin.top });
    fabHelper.update();
}