<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Orderlistinfo.aspx.cs" Inherits="ZoomLaCMS.Manage.Shop.Orderlistinfo"  MasterPageFile="~/Manage/I/Default.master" %>
...
<div class="tab-content" style="width:100%;margin-top:0px;">
    <div role="tabpanel" class="tab-pane active" id="OrderState">
        <table class="table table-striped table-bordered">
            ...
            <tr>
                <td class="text_md">收货人：<%:conMod.FullName %></td>
                <td>邮箱地址：<%:conMod.Email %></td>
            ...
            <tr>
                ...
                <td>物流状态：
                    <asp:Label ID="ExpStatus_L" runat="server"></asp:Label>
                    <a target="_blank" runat="server" id="SearchExp_A" visible="false" class="btn btn-info btn-sm">物流查询</a>
                </td>
            </tr>
            <tr>
                <td>订单邮件</td>
                <td>付款邮件：
                    <%=DataConverter.CLng(EmailDR["Email_Pay"])==1?ComRE.Icon_OK:ComRE.Icon_Error %>
                    <asp:Button runat="server" ID="SendMail" CommandName="pay" OnClick="SendMail_Click" Text="发送邮件" class="btn btn-sm btn-info" OnClientClick="return confirm('确定要发送邮件吗');"/>
                </td>
                <td>货运邮件：
                    <%=DataConverter.CLng(EmailDR["Email_Exp"])==1?ComRE.Icon_OK:ComRE.Icon_Error %>
                    <asp:Button runat="server" ID="SendMail2" CommandName="exp" OnClick="SendMail_Click" Text="发送邮件" class="btn btn-sm btn-info" OnClientClick="return confirm('确定要发送邮件吗');"/>
                </td>
                <td>
                    售后邮件：
                    <%=DataConverter.CLng(EmailDR["Email_After"])==1?ComRE.Icon_OK:ComRE.Icon_Error %>
                    <asp:Button runat="server" ID="SendMail3" CommandName="after" OnClick="SendMail_Click" Text="发送邮件" class="btn btn-sm btn-info" OnClientClick="return confirm('确定要发送邮件吗');"/>
                </td>
            </tr>
            <tr><td>订单备注</td><td colspan="3"><%:orderinfo.Ordermessage %></td></tr>
            <tr>
                <td>订单状态</td>
                <td colspan="3">
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                <table class="table table-striped table-bordered">
                    ...
                    <asp:Repeater ID="Procart_RPT" runat="server" OnItemDataBound="Procart_RPT_ItemDataBound">
                        <ItemTemplate>
                            <tr>
                                <td><%#Eval("ProID") %></td>
                                <td class="text-center">
                                    <img src="<%#GetImage() %>" style="width: 160px; height: 100px;" class="design_img" />
                                    <div style="margin-top: 5px;">
                                        <a href="/Plugins/Third/Logo/DesignShow.aspx?CartID=<%#Eval("ID") %>" class="btn btn-info btn-sm" target="_blank">查看设计</a>
                                    </div>
                                </td>
                                <td>
                                    <div><%#Eval("ProName") %></div>
...
<%--    <tr>
        <td>设计模板</td>
        <td>
            <a onClick="showLogoTip();" id="logo_tip" class="btn btn-info">查看设计</a>
        </td>
    </tr>--%>
    <tr>
...
<script>
    ...
    {
        var url = siteconf.path + "/Plus/Logo/Design.aspx?ID=<%=orderinfo.Promoter%>";
        ShowComDiag(url,"模板设计");
    }
</script>
</asp:Content>
